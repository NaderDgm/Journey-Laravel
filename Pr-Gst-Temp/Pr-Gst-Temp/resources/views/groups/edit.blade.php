...
@section('content')
    <div class="mx-auto max-w-3xl">
        <h1 class="text-2xl font-semibold text-slate-900">Modifier un groupe</h1>
        <p class="mt-1 text-sm text-slate-600">Mettez à jour les informations du groupe.</p>

        <form action="{{ route('groups.update', $group) }}" method="POST" class="mt-6 space-y-6 rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
            @csrf
            @method('PUT')
